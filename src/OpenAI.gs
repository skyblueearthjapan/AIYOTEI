...
## titleについて（重要）
- titleには「何をするか」「どこに行くか」などの**主な予定内容**を入れる
- 入力テキストの中心となる内容・行動・イベント名をtitleにする
- 「予定」という汎用的な文字は使わない（具体的な内容を抽出する）
...
## memoについて
- memoは**補足的な情報のみ**を入れる
- 「持っていくもの」「準備すること」「注意事項」などがmemo
- 主な予定内容はtitleに入れ、memoには入れない
- 補足情報がなければ null にする

## その他ルール
1. 時間が曖昧な場合は start_time/end_time を null にする
